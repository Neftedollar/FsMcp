...
let server = mcpServer {
    ...
    tool (TypedTool.define<EchoArgs> "echo" "Echoes the message back" (fun args _ -> task {
        return Ok [ Content.text $"Echo: {args.message}" ]
    }) |> unwrapResult)

    tool (TypedTool.define<ReverseArgs> "reverse" "Reverses the text" (fun args _ -> task {
    ...
    resource (
        Resource.define "info://server/status" "Server Status" (fun _ _ -> task {
